...
namespace YouTube_Downloader
{
    public partial class MainForm : Form
    {
        ...
        private void backgroundWorker1_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                if (urls.Count > 0)
                {
                    ...
                    name_Label2.Text = FormatTitle(urls[0].VideoTitle);
                }
                else
                {
                    drawVideoLenght("");
                ...
                quality_ComboBox.Enabled = url_TextBox.Enabled = true;
        ...
        private void get_Button_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Helper.IsValidUrl(url_TextBox.Text) ||
                    !url_TextBox.Text.ToLower().Contains("www.youtube.com/watch?"))
                    MessageBox.Show(this,
                        @"You enter invalid YouTube URL, Please correct it.Note: URL should start with:http://www.youtube.com/watch?",
                        @"Invalid URL", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                else
                {
                    get_Button.Enabled = url_TextBox.Enabled = false;
                    progressBar1.Visible = true;
                    video_PictureBox.ImageLocation = string.Format("http://i3.ytimg.com/vi/{0}/default.jpg",
                        YouTubeDownloader.GetVideoIDFromUrl(url_TextBox.Text));
